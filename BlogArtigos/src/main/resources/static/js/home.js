document.addEventListener('DOMContentLoaded', () => {
    ...
    const novoArtigoFormContainer = document.getElementById('form-novo-artigo');
    ...
    // --- Lógica de Novo Artigo (COM CORREÇÃO DE ERRO) ---
    novoArtigoForm.addEventListener('submit', async function(event) {
        ...
        try {
            ...
            if (response.status === 403) {
                // Erro de Permissão (o que você estava a ver)
                throw new Error('Acesso negado. Você não tem permissão de AUTOR.');
            }
            if (!response.ok) {
                // Outro erro (Ex: 500 - Duplicate Entry)
                const errorText = await response.text(); // Pega o erro do backend
                console.error("Erro do Backend:", errorText);
                throw new Error('Erro ao publicar o artigo. (O servidor falhou)');
            }
            // --- FIM DA CORREÇÃO ---
    ...
    // --- Carregador Principal de Artigos ---
    ...
    async function deletarArtigo(event) {
        ...
        try {
            ...
            await carregarArtigos(searchBar.value);
